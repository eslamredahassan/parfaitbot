            ...
            if (admin.roles.cache.has(config.devRole)) {
              ...
              await ap_user.send({
                embeds: [
                  new MessageEmbed()
                    .setColor(`${color.gray}`)
                    .setTitle(`${emojis.forb} Banned`)
                    .setImage(`${banners.bannedBanner}`)
                    .setDescription(
                      `You have been banned from the recruitments application system for breaking the rules`
                    ),
                ],
              });
              ...
              await interaction
                .reply({
                  embeds: [
                    {
                      title: `${emojis.forb} Banned Alert`,
                      description: `${emojis.whiteDot} You banned ${ap_user} from the recruitments application system\n${emojis.whiteDot} Removed his application from pin list\n${emojis.whiteDot} His thread will be automatically archived in \`\`20 Seconds\`\``,
                      color: `${color.gray}`,
                    },
                  ],
                ...
                .catch(() => console.log('Error Line 771'));

              await ap_user.roles.remove(config.waitRole).catch(() => console.log('Error Line 2773'));

              await ap_user.roles.add(config.banRole).catch(() => console.log('Error Line 2775'));

              console.log(
                `\x1b[31m 🛠`,
                `\x1b[30m ${moment(Date.now()).format("lll")}`,
                `\x1b[33m Sun wannabe role REMOVED`,
                `\x1b[33m Ban role ADDED`
              );
              ...
              /// Rename The Thread ///
              await threadName.setName("🧤︱" + `${userName}` + " Banned");
              /// Lock the thread ///
              ...
              await interaction
                ...
                .catch(() => console.log('Error Line 2816'));
              console.log(
                `\x1b[31m 🛠`,
                `\x1b[30m ${moment(Date.now()).format("lll")}`,
